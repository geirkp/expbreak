...
To find a measure of the irregularities present in the PIV measurements, the standard deviations are calculated at the different FOV for the strongest plunging breaker. Deviations are extracted at times where the mean flow in an area near the beach (z=[0 - 0.6]cm) has a specified velocity ( [40, 0, -40]cm/s).

\begin{equation}
\sigma=\sqrt{\frac{1}{N-1}\sum_{i=1}^{N}(x_i-\overline{x})^2} 
\end{equation}

The average deviations in the z-direction are calculated from the same area (z=[0 - 0.6]cm), and M corresponds to number of vertically grid-points within this area.
...
More details on both models are given in  \cite{pedersen2013runup}. 
 ...
 Simulations are conducted with a range of various spatial resolutions. Grid refinements are engaged up to the point where the runup height and time was deterministic with a 0.2\% error. The same resolutions was applied to the breaking waves, and the finest resolution $\delta x'$ was approximately $0.07H$.  For all the waves the temporal increment was set to 0.0073s, which is twice as large as the temporal averaging used in PIV. The viscous boundary layer model generated 600 grid points along the beach, with a spatial increment of $0.0042 H$. The time resolution was kept the same as for BIM.
...
\begin{table}[]
\caption{\textit{Irregulariety for $\alpha=0.50$, the men deviation from z=0-0.6 for the three repetitions }}
\centering
\begin{tabular}{ccccccc}
\hline
$\overline{\sigma}$  $[\frac{cm}{s}]$& $  40\frac{cm}{s}$  &$ 0\frac{cm}{s}$ & $-40\frac{cm}{s}$   \\ \hline
\textit{FOV I}  & 0.52   &    0.24  & 0.51                       \\
\textit{FOV II}  & 1.03   &    0.56   & 1.06                    \\
\textit{FOV III} &  2.10   &    1.03 & 0.75                            \\
\textit{FOV IV}  & 3.48    &   1.70  & 0.79                 \\
\end{tabular}
\label{tab:irr}
\end{table}
