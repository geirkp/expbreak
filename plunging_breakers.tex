...
In fact, in Figure \ref{fig:max_runup} we observe transverse variations in the field of views, and the average FOV
runup height for the smallest wave $(\alpha=0.10)$ is 4.10. This \marginpar{Endring} is more than 3.76\% smaller than the maximum one, but still presumably larger than the true average runup height due to lack of information near the side walls of the wave tank. This implies that real difference between theoretical and computed runup is larger than indicated by the maximum values. Table \ref{tab:max_shore} shows that the maximum runup is fairly repeatable for all waves including the breaking ones. % An image of shoreline at maximum runup for case 10 is shown in \ref{fig:overflate_10_bilde}.  
