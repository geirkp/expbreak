...
Nominally, the amplitude to depth ratios, $\alpha=A/H$, should equal  (0.1,0.12,0.2,0.3,0.4,0.5).  
...
An acoustic wave gauge (ultra Banner U-Gage S18U, sample frequency of 200Hz) measured the wave height at the toe of the beach and the amplitudes were found to be $A/H=(0.0989, 0.1191, 0.1981, 0.2958, 0.3939, 0.4874)$. \sidenote{Sett inn riktig amplitude som er gitt av BIM}  
...
The scatter $\delta_i$ for some measured quantity, $x_i$, is then calculated as,
\begin{equation}
\delta_i=\frac{x_i-\overline{x}}{\overline{x}}
\end{equation}
where $\overline{x}$ is the mean over the  repetitions and

To find a measure for irregularities of the velocities in the swash tongue, the standard deviation for the the three repepitions are calculated, at the differents FOV. Deviations are extracted at times where the mean flow in an area from z=(0 - 0.6)cm is [40, 0, 40]cm/s.
\begin{equation}
\sigma=\frac{1}{N}\sum\sqrt{(x_i-\overline{x})^2}
\end{equation}
The average deviations are also calucuate from the area of z=(0 - 0.6)cm. Here M corresponds to number of gridpoints in z-direction in this area.
\begin{equation}
\overline{\sigma}=\frac{1}{M}\sum(\sigma_j)  \quad j=1..M
\end{equation}
